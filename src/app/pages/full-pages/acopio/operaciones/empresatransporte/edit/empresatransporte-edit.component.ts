...
export class EmpresaTransporteEditComponent implements OnInit {
    ...
    ConsultarPorId() {
        this.spinner.show();
        this.empresaTransporteService.ConsultarPorId({ ProductoPrecioDiaId: this.vId }).subscribe((res: any) => {
    ...
    async CompletarFormulario(data: any) {
        ...
        if (data.MonedaId) {
            this.empresaTransporteEditForm.controls.moneda.setValue(data.MonedaId);
        }
        if (data.ProductoId) {
            this.empresaTransporteEditForm.controls.producto.setValue(data.ProductoId);

          
        }
        if (data.SubProductoId) {
            this.empresaTransporteEditForm.controls.subproducto.setValue(data.SubProductoId);
        }
        if (data.PrecioDia) {
            this.empresaTransporteEditForm.controls.precioDia.setValue(data.PrecioDia);
        }
        if (data.EstadoId) {
            this.empresaTransporteEditForm.controls.estado.setValue(data.EstadoId);
        }
        if (data.FechaRegistro) {
            this.empresaTransporteEditForm.controls.fecRegistro.setValue(data.FechaRegistro.substring(0, 10));
        }
